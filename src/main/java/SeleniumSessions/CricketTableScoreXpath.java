...
public class CricketTableScoreXpath {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		List<WebElement> scoreList = driver
				.findElements(By.xpath("(//a[text()='Omaima Sohail'])/parent::td/following-sibling::td"));
		for (int i = 1; i < scoreList.size(); i++) {
			System.out.println(scoreList.get(i).getText());
		}
		
		System.out.println(getWicketTakerName("Omaima Sohail"));
		System.out.println(getScoreList("Omaima Sohail"));

		System.out.println(getWicketTakerName("Nida Dar"));
		System.out.println(getScoreList("Nida Dar"));
	}
